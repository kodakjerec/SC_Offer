...
namespace SC_DAO
{
    public class XMSSCData
    {
        ...
        #region SC日結帳務
        public string SC_GetCloseDate(string strCon)
        {
            string strdate = "";
            string Create_Assign = "Select S_bsda_FieldId from [3PL_BaseData] where S_bsda_CateId='CloseData'";
            Hashtable ht_assign = new Hashtable();
            Hashtable ht_trash = new Hashtable();

            DataSet ds1 = IO.SqlQuery(strCon, Create_Assign, ht_assign);
            strdate = ds1.Tables[0].Rows[0][0].ToString();
